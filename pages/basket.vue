<template>
  <div class="lg:flex max-w-container mx-auto">
    <!-- 장바구니 정보 -->
    <div>
      <!-- 아이템 -->
      <div
      ...
      <div class="border p-[1rem]">
        ...
        <div class="flex flex-col gap-1 my-[1rem]">
          ...
          <div class="flex justify-between">
            ...
            <p>{{ formatToWon(paymentInfo.price) }}원</p>
          </div>
          <div class="flex justify-between">
            <p>할인 금액</p>
            <p>{{ formatToWon(paymentInfo.sale) }}원</p>
          </div>
          <div class="flex justify-between">
            <p>배송비</p>
            <p>{{ formatToWon(paymentInfo.parcel) }}원</p>
          </div>
        </div>
        <hr />
        <div
          class="my-[1rem] text-[1.2rem] font-bold flex justify-between gap-2"
        >
          <p>총결제금액</p>
          <p>{{ formatToWon(paymentInfo.total) }}원</p>
        </div>
        <button
          @click="onPayItem"
...
const onPayItem = () => {
  console.log("결제 ", paymentInfo.total);
};
